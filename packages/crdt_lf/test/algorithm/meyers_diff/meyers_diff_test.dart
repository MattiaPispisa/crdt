...
// TODO(mattia): groups: segments result, segments ordering, segments contiguous, 
// diff application to old test
...
void main() {
  group('myersDiff', () {
    ...
    test('apply diff operations with unicode', () {
      const String oldText = 'Emoji test: 😀😎🎉';
      const String newText = 'Emoji test: 😀🤖🎊🎉';
    ...
    test('segments are contiguous', () {
      const String oldText = 'ABCDEFGH';
      const String newText = 'AXCXEFXH';

      final diffs = myersDiff(oldText, newText);

      // Verify segments cover all of oldText and newText without gaps
      int expectedOldPos = 0;
      int expectedNewPos = 0;
...
/// This function applies removals and insertions using the indices.
String _applyDiff(String oldText, List<DiffSegment> diffs) {
  final result = StringBuffer();

  // We'll process segments in order, tracking our position in oldText
  int oldPos = 0;

  for (final seg in diffs) {
    // Skip any characters between oldPos and seg.oldStart
    // (these might have been removed by previous operations, but we track by oldStart)
    if (seg.oldStart > oldPos) {
      // This should not happen if segments are contiguous
      result.write(oldText.substring(oldPos, seg.oldStart));
      oldPos = seg.oldStart;
    }

    switch (seg.op) {
      case DiffOp.equal:
        // Copy the equal text from oldText
        result.write(oldText.substring(seg.oldStart, seg.oldEnd));
        oldPos = seg.oldEnd;
        break;
      case DiffOp.insert:
        // Insert new text (oldStart == oldEnd for inserts)
        result.write(seg.text);
        // oldPos stays the same since we're inserting
        break;
      case DiffOp.remove:
        // Skip the removed text in oldText
        oldPos = seg.oldEnd;
